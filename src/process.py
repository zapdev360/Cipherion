...
class ProcessHandler:
    ...
    def hashrphrase(self, phrase: str) -> bytes:
        return hashlib.sha256(phrase.encode()).digest()


    def encrypt(self, ptext, algo='AES', suppress=False):
        if not suppress:
    ...
    def decrypt(self, ctext, key, algo='AES', rotate=False, suppress=False):
        if not suppress:
        ...
        if rotate:
            newkey, newencdata = self.encrypt(ptext.decode('utf-8'), algo, suppress=True)
            return ptext.decode('utf-8'), newkey, newencdata

        return ptext.decode('utf-8')


    def rotate(self, ctext, oldkey, algo='AES'):
        decdata = self.decrypt(ctext, oldkey, algo, rotate=True, suppress=True)
        return decdata
